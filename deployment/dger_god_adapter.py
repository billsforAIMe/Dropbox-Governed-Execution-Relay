...
def cwd(pid):
    ...
    actual=Path(names[0])
    _physical_dir(actual)
    return actual

def _argv(command,pid):
    try: argv=shlex.split(command)
    except ValueError: raise RuntimeError(f"process argv parse failed:{pid}")
    if not argv: raise RuntimeError(f"process argv empty:{pid}")
    return argv

def _physical_argv_paths(command,pid):
    out=[]
    for token in _argv(command,pid):
        p=Path(token)
...
def _interpreter_owned(pid, expected_cwd):
    root=_physical_dir(Path(expected_cwd))
    venv=_physical_dir(root/".venv")
    bindir=_physical_dir(venv/"bin")
    candidates=[]
...
def _owned(pid, command, expected_cwd, required_target):
    root=_physical_dir(expected_cwd)
    actual_cwd=cwd(pid)
    if not os.path.samefile(actual_cwd,root):
        raise RuntimeError(f"process cwd ownership mismatch:{pid}:{actual_cwd}:{root}")
    target=_physical_file(required_target)
    matched=[]
    for token_path in _physical_argv_paths(command,pid):
        try:
            if os.path.samefile(token_path,target): matched.append(str(token_path))
        except (FileNotFoundError,OSError): pass
    if len(matched)!=1:
        raise RuntimeError(f"process target ownership mismatch:{pid}:{target}:{matched}")
    exe=_interpreter_owned(pid,root)
    return {"pid":pid,"command":command,"cwd":str(actual_cwd),"target":str(target),"executable":exe}

def procs():
    rows=_ps_rows(); runtime=[]; writers=[]; classified=set()
    rt=STATE/"runtime/current"; rt_target=rt/"scripts/dger.py"
    lpid=_launchctl_pid()
    marker=str(STATE/"attempts")+"/"; suffix="/gep-tree/scripts/governed_exec.py"
    legacy=str(LEGACY)
    for pid,cmd in rows:
        argv=_argv(cmd,pid)
        abs_tokens=[x for x in argv if x.startswith("/")]
        runtime_tokens=[x for x in abs_tokens if x==str(rt_target)]
        writer_tokens=[x for x in abs_tokens if x.startswith(marker) and x.endswith(suffix)]
        legacy_tokens=[x for x in abs_tokens if x==legacy]
        if runtime_tokens:
            if len(runtime_tokens)!=1 or lpid is None or pid!=lpid: raise RuntimeError(f"unowned DGER runtime process:{pid}")
            runtime.append(_owned(pid,cmd,rt,rt_target)); classified.add(pid)
        if writer_tokens:
            if len(writer_tokens)!=1: raise RuntimeError(f"writer target ambiguous:{pid}")
            raw_target=Path(writer_tokens[0]); _no_symlink_components(raw_target)
            target=_physical_file(raw_target); gep_tree=_physical_dir(target.parent.parent)
            attempts=_physical_dir(STATE/"attempts")
            try: target.relative_to(attempts)
            except ValueError: raise RuntimeError(f"writer target ownership mismatch:{pid}:{target}")
            writers.append(_owned(pid,cmd,gep_tree,target)); classified.add(pid)
        if legacy_tokens: raise RuntimeError(f"legacy DGER runtime process active:{pid}")
        plausible=[x for x in abs_tokens if Path(x).name in ("dger.py","governed_exec.py","dropbox-governed-execution-relay")]
        recognized=set(runtime_tokens+writer_tokens+legacy_tokens)
        unowned=[x for x in plausible if x not in recognized]
        if unowned: raise RuntimeError(f"plausible unowned DGER/GEP process:{pid}:{unowned}")
    if lpid is not None and not runtime:
        cmd=next((c for p,c in rows if p==lpid),None)
        if cmd is None: raise RuntimeError(f"launchctl pid absent from ps:{lpid}")
        runtime=[_owned(lpid,cmd,rt,rt_target)]; classified.add(lpid)
