...
def _ps_pids():
    # pid/uid output is enumeration/filter-only. Same-user ownership is re-established
    # from native proc_bsdinfo inside _owned() and never trusted from this rendering.
...
def _libproc():
    if sys.platform!="darwin": raise RuntimeError("native Darwin process identity unavailable")
    return ctypes.CDLL("/usr/lib/libproc.dylib",use_errno=True)

def _bsd_identity(pid):
    lib=_libproc(); fn=lib.proc_pidinfo
    ...
    if got!=ctypes.sizeof(info):
        ...
        raise RuntimeError(f"proc_bsdinfo failed:{pid}:{got}:{err}")
    if int(info.pbi_pid)!=int(pid): raise RuntimeError(f"proc_bsdinfo pid mismatch:{pid}:{info.pbi_pid}")
...
def _argv_from_procargs(raw,pid):
    ...
    if len(argv)!=argc or any(not isinstance(x,str) for x in argv): raise RuntimeError(f"native argv malformed:{pid}")
    return argv
...
def _physical_file(path):
    p=Path(path); _no_symlink_components(p); st=p.lstat()
...
def _writer_role_argv(argv, target):
    if not isinstance(argv,list) or not isinstance(target,str): return False
    hits=[i for i,x in enumerate(argv) if x==target]
    if len(hits)!=1: return False
    i=hits[0]
    return argv[i:]==[target,"self-check","ai-me"]
...
def _classify_argv(pid,argv,lpid):
    ...
    runtime=[x for x in abs_tokens if x==rt_target]
    writer_candidates=[x for x in abs_tokens if x.startswith(marker) and x.endswith(suffix)]
    if len(writer_candidates)>1: raise RuntimeError(f"writer target ambiguous:{pid}")
    writers=[]
    if writer_candidates:
        candidate=writer_candidates[0]
        if not _writer_role_argv(argv,candidate): raise RuntimeError(f"writer argv operation mismatch:{pid}:{candidate}")
        writers=[candidate]
    legacy_hits=[x for x in abs_tokens if x==legacy]
...
def _owned(pid, expected_cwd, required_target, role, launchctl_pid):
    if role not in ("runtime","writer"): raise RuntimeError(f"process role invalid:{role}")
    before=_bsd_identity(pid)
    if before["uid"]!=os.getuid(): raise RuntimeError(f"process uid ownership mismatch:{pid}:{before['uid']}")
    # Native argv is acquired only after the first process-incarnation snapshot.
    argv=_native_argv(pid)
    cls=_classify_argv(pid,argv,launchctl_pid)
    target=_physical_file(required_target)
    wanted=str(target)
    if role=="runtime":
        if cls["runtime_target"]!=wanted or cls["writer_target"] is not None: raise RuntimeError(f"process role/target ownership mismatch:{pid}:runtime")
    else:
        if cls["writer_target"]!=wanted or cls["runtime_target"] is not None or not _writer_role_argv(argv,wanted): raise RuntimeError(f"process role/target/operation ownership mismatch:{pid}:writer")
    ...
    if before!=after: raise RuntimeError(f"process PID reused during ownership proof:{pid}")
    return {"pid":pid,"uid":before["uid"],"ppid":before["ppid"],"start_sec":before["start_sec"],"start_usec":before["start_usec"],"argv":list(argv),"cwd":str(actual_cwd),"target":str(target),"executable":exe,"role":role}

def _same_incarnation(a,b):
    keys=("pid","uid","ppid","start_sec","start_usec")
    return isinstance(a,dict) and isinstance(b,dict) and all(a.get(k)==b.get(k) for k in keys)
...
def procs():
    ...
    for pid,_uid in rows:
        # Preliminary argv/classification is filter-only. _owned() takes a fresh native
        # process identity and re-reads/reclassifies argv inside its guarded interval.
        try: preliminary_argv=_native_argv(pid)
        except ProcessLookupError: continue
        cls=_classify_argv(pid,preliminary_argv,lpid)
        if cls["runtime_target"]: runtime.append(_owned(pid,rt,rt_target,"runtime",lpid))
        if cls["writer_target"]:
            ...
            writers.append(_owned(pid,gep_tree,target,"writer",lpid))
    if lpid is not None and not runtime:
        ...
        runtime=[_owned(lpid,rt,rt_target,"runtime",lpid)]
...
def stop():
    ...
    r,w=procs()
    if r: raise RuntimeError("DGER runtime still active after launchctl bootout")
    if w:
        ids=[{"pid":x.get("pid"),"start_sec":x.get("start_sec"),"start_usec":x.get("start_usec"),"target":x.get("target")} for x in w]
        raise RuntimeError(f"owned GEP writer still active; PID-only termination forbidden:{ids}")
...
def dbroot():
    ...
    if base.is_symlink() or not base.is_dir(): raise RuntimeError("Dropbox root unavailable:CloudStorage")
    provider=base/"Dropbox"; software=provider/"Software"; marker=software/"NSP - Temporary Files"
    for label,path in (("Dropbox",provider),("Software",software),("NSP - Temporary Files",marker)):
        if path.is_symlink() or not path.is_dir(): raise RuntimeError(f"Dropbox root unavailable:{label}")
    resolved=provider.resolve()
    if resolved.parent!=base.resolve(): raise RuntimeError("Dropbox root escape")
    return resolved
