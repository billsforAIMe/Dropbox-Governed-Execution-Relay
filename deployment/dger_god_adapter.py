...
def _owned(pid, expected_cwd, required_target, role, launchctl_pid):
    ...
    if role=="runtime":
        ...
        if cls["writer_target"]!=wanted or cls["runtime_target"] is not None: raise RuntimeError(f"process role/target ownership mismatch:{pid}:writer")
...
def stop():
    ...
    for item in w:
        pid=int(item["pid"])
        # Re-establish the full guarded ownership proof immediately before signaling;
        # a stale/reused PID record is never sufficient authority for SIGTERM.
        fresh=_owned(pid,Path(item["cwd"]),Path(item["target"]),"writer",lpid)
        if not _same_incarnation(item,fresh): raise RuntimeError(f"writer PID incarnation changed before termination:{pid}")
