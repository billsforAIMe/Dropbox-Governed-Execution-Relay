...
adapter_spec=importlib.util.spec_from_file_location("dger_god_adapter_c7q",ROOT/"deployment/dger_god_adapter.py")
...
relay_spec=importlib.util.spec_from_file_location("dger_relay_c7q",ROOT/"src/dger/relay.py")
...
class C7QIdentityTests(unittest.TestCase):
    ...
    def test_adoption_doc_exact_guarded_recovery_contract(self):
        ...
        self.assertNotIn("adopts only a live process whose command contains",s)
    def test_adapter_prefilter_pid_reuse_blocks_procs_and_stop_signal(self):
        with tempfile.TemporaryDirectory(prefix="DGER Adapter PID reuse space ",dir="/private/tmp") as td:
            ...
            pid=777; old_argv=[str(py),str(target)]; replacement_argv=[str(py),"-c","print('replacement')"]
            ...
            with mock.patch.object(adapter,"STATE",state),mock.patch.object(adapter,"_ps_pids",return_value=[(pid,os.getuid())]),mock.patch.object(adapter,"_launchctl_pid",return_value=None),mock.patch.object(adapter,"_native_argv",side_effect=[old_argv,replacement_argv]),mock.patch.object(adapter,"_bsd_identity",return_value=replacement),mock.patch.object(adapter,"_lsof_names",side_effect=names):
                with self.assertRaisesRegex(RuntimeError,"role/target ownership"): adapter.procs()
            ...
            with mock.patch.object(adapter,"STATE",state),mock.patch.object(adapter,"run",return_value=boot),mock.patch.object(adapter,"_ps_pids",return_value=[(pid,os.getuid())]),mock.patch.object(adapter,"_launchctl_pid",return_value=None),mock.patch.object(adapter,"_native_argv",side_effect=[old_argv,replacement_argv]),mock.patch.object(adapter,"_bsd_identity",return_value=replacement),mock.patch.object(adapter,"_lsof_names",side_effect=names),mock.patch.object(adapter.os,"kill") as kill:
                with self.assertRaisesRegex(RuntimeError,"role/target ownership"): adapter.stop()
                kill.assert_not_called()
            # Even a previously guarded writer record is re-established immediately before signal.
            ...
            with mock.patch.object(adapter,"run",return_value=boot),mock.patch.object(adapter,"procs",return_value=([],[old])),mock.patch.object(adapter,"_launchctl_pid",return_value=None),mock.patch.object(adapter,"_bsd_identity",return_value=replacement),mock.patch.object(adapter,"_native_argv",return_value=replacement_argv),mock.patch.object(adapter.os,"kill") as kill:
                with self.assertRaises(RuntimeError): adapter.stop()
                kill.assert_not_called()
