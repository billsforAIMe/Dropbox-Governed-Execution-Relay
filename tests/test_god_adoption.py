...
class T(unittest.TestCase):
 ...
 def test_pyrunway_adapter(self):
  s=(ROOT/"deployment/dger-god-adapter").read_text(); self.assertIn("/usr/local/bin/pyrunway --standalone",s); self.assertNotIn("/usr/bin/python",s); a=(ROOT/"deployment/dger_god_adapter.py").read_text(); self.assertIn("/usr/local/bin/uv",a); self.assertIn("UV_OFFLINE",a); self.assertIn("--locked",a)
 def test_stable_launcher(self):
  s=(ROOT/"launcher/dropbox-governed-execution-relay").read_text(); self.assertIn(str(STATE/"runtime/current/scripts/dger.py"),s); self.assertNotIn("delivered-identity.json",s); self.assertIn("/usr/local/bin/pyrunway",s); self.assertNotIn("--standalone",s)
 def test_plist(self):
