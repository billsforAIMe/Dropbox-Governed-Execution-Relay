...
_GEP_RESULT_KEYS = {
    "ok", "code", "launch_capability", "origin", "service_deployment_id",
    "ahc_execution_claim_id", "ahc_effect_reservation_id", "ahc_work_revision",
    "gep_execution_id", "gep_request_digest", "gep_admission_sha256",
    "origin_envelope_sha256", "trusted_binding_digest", "result_binding",
}
_GEP_ORIGIN_KEYS = {
    "tenant_id", "principal_id", "deployment_id", "fleet_epoch",
    "originating_invocation_id", "context_digest",
}
_AHC_RESULT_KEYS = {
...
_CHM_RESULT_KEYS = {
    "ok", "code", "handoff_id", "namespace_binding", "project_binding",
    "external_execution", "terminal_result", "execution_authority",
    "authorizes_moh_execution", "ahc_effect_truth", "gep_execution_admission",
}
...
class AuthenticatedCorrelationReader(Protocol):
    ...
    Implementations authenticate the DGER EXECUTION_RELAY service and exact
    non-transferable GTG delegated context before invoking a peer. Caller/Dropbox
    identity is never sufficient at this boundary.
    """
...
def _same_origin_service(a: DelegatedServiceContext, b: DelegatedServiceContext) -> bool:
    fields = (
        "tenant_id", "principal_id", "deployment_id", "origin_actor_role",
        "provisioned_fleet_epoch", "origin_context_digest", "origin_invocation_id",
        "service_role", "service_id", "service_deployment_id", "project_binding",
    )
    return all(getattr(a, field) == getattr(b, field) for field in fields)


class Gep14CorrelationPeers:
    """Supply only the source-delivered GEP Gen14 correlation proposition.

    Every non-correlation peer method delegates unchanged to ``base_peers``; this
    wrapper cannot alter AHC/MOH/CHM effect ordering or DGER recovery semantics.
    """
    ...
    def _read(
        ...
        context = parse_delegated_service_context(
            _mapping(observed.delegated_context, "DELEGATED_CONTEXT_INVALID")
        )
        ...
        exact = f"tool:{tool_id}:{operation}"
        if exact not in context.authorized_operations:
            raise DgerGen4Error("DELEGATED_CONTEXT_OPERATION_NOT_AUTHORIZED", exact)
        evidence = _validate_invocation_evidence(
            observed.invocation_evidence,
            expected_tool=tool_id,
            expected_operation=operation,
        )
        result = _mapping(observed.result, "PEER_READ_RESULT_INVALID")
    ...
    def _request(
        ...
        expected = {
            "schema", "dger_request_id", "gep_execution_id",
            "ahc_effect_reservation_id", "chm_handoff_id",
        }
        ...
        execution_id = request.get("gep_execution_id")
        ...
        return execution_id, effect_id, handoff_id
    ...
    def _gep(
        ...
        if result.get("code") != _GEP_CORRELATION_CODE or result.get("launch_capability") is not False:
            raise DgerGen4Error("GEP_CORRELATION_RESULT_INVALID")
        if result.get("result_binding") is not None:
            raise DgerGen4Error("GEP_RESULT_ALREADY_BOUND")
        origin = _mapping(result.get("origin"), "GEP_CORRELATION_ORIGIN_INVALID")
        ...
        if result.get("gep_execution_id") != execution_id:
        ...
        for value, code in (
            ...
            (result.get("gep_admission_sha256"), "TRUSTED_ADMISSION_DIGEST_INVALID"),
            (result.get("origin_envelope_sha256"), "GEP_ORIGIN_ENVELOPE_DIGEST_INVALID"),
            (result.get("trusted_binding_digest"), "GEP_TRUSTED_BINDING_DIGEST_INVALID"),
        ):
    ...
    def _ahc(
        ...
        *,
        ...
        if isinstance(epoch, bool) or not isinstance(epoch, int) or epoch < 1:
            raise DgerGen4Error("AHC_EFFECT_READ_RESULT_INVALID")
        if result.get("terminal_evidence_digest") is not None or result.get("replayed") is not False:
            raise DgerGen4Error("AHC_EFFECT_READ_RESULT_INVALID")
        _safe_id(result.get("effect_class"), "AHC_EFFECT_READ_RESULT_INVALID")
        _safe_id(result.get("workstream_id"), "AHC_EFFECT_READ_RESULT_INVALID")
        checks = (
            (result.get("effect_id"), effect_id, "AHC_EFFECT_MISMATCH"),
            (result.get("execution_claim_id"), claim_id, "AHC_CLAIM_MISMATCH"),
            (str(revision), work_revision, "AHC_WORK_REVISION_MISMATCH"),
            (epoch, gep_context.provisioned_fleet_epoch, "AHC_FLEET_EPOCH_MISMATCH"),
            ...
            (result.get("gep_execution_id"), execution_id, "GEP_EXECUTION_MISMATCH"),
            (result.get("gep_request_digest"), request_digest, "GEP_REQUEST_DIGEST_MISMATCH"),
        )
    ...
    def _chm(
        ...
        *,
        ...
        if result.get("namespace_binding") != expected_namespace or result.get("project_binding") != gep_context.project_binding:
            raise DgerGen4Error("CHM_ORIGIN_CONTEXT_MISMATCH")
        if result.get("handoff_id") != handoff_id:
            raise DgerGen4Error("CHM_HANDOFF_CORRELATION_MISMATCH")
        if result.get("external_execution") != {
    ...
    def establish_correlation(
        ...
        execution_id, effect_id, handoff_id = self._request(
            request, admission_sha256, payload_manifest_sha256
        )
        gep_result, gep_context, gep_evidence = self._read(
            GEP_TOOL_ID, GEP_CORRELATION_OPERATION, {"execution_id": execution_id}, service_identity
        )
        origin, claim_id, work_revision, request_digest = self._gep(
            gep_result, gep_context, service_identity, execution_id, effect_id, admission_sha256
        )
        ...
        ahc_result, ahc_context, ahc_evidence = self._read(
            ...
            AHC_EFFECT_READ_OPERATION,
            {
                ...
                "effect_id": effect_id,
                "gep_execution_id": execution_id,
                "gep_request_digest": request_digest,
            },
            service_identity,
        )
        self._ahc(
            ahc_result, ahc_context, gep_context,
            claim_id=claim_id, effect_id=effect_id, work_revision=work_revision,
            execution_id=execution_id, request_digest=request_digest,
        )
        evidence = [gep_evidence, ahc_evidence]
        if handoff_id is not None:
            chm_result, chm_context, chm_evidence = self._read(
                CHM_TOOL_ID, CHM_HANDOFF_READ_OPERATION, {"handoff_id": handoff_id}, service_identity
            )
            self._chm(
                chm_result, chm_context, gep_context,
                handoff_id=handoff_id, effect_id=effect_id, execution_id=execution_id,
            )
            evidence.append(chm_evidence)
        body = {
        ...
        _validate_correlation(correlation, request, admission_sha256, payload_manifest_sha256, service_identity)
        return correlation

    def stage_moh(self, correlation: TrustedCorrelation, frozen_stage: Any, payload_manifest_sha256: str) -> StageReceipt:
    ...
    def ahc_accept_terminal(
        self, correlation: TrustedCorrelation, terminal_digest: str, result_ref: str, result_sha256: str
    ) -> Ack:
