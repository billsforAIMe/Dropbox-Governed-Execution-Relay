...
def resolve_dropbox_root() -> Path:
    ...
    if base.is_symlink() or not base.is_dir():
        raise RuntimeError("DROPBOX_ROOT_UNAVAILABLE:CloudStorage")
    provider = base / "Dropbox"
    ...
    for label, path in (("Dropbox", provider), ("Software", software), ("NSP - Temporary Files", marker)):
        if path.is_symlink() or not path.is_dir():
            raise RuntimeError(f"DROPBOX_ROOT_UNAVAILABLE:{label}")
    resolved = provider.resolve()
    if resolved.parent != base.resolve():
        raise RuntimeError("DROPBOX_ROOT_ESCAPE")
    return resolved
...
def _argv_from_procargs(raw:bytes,pid:int)->list[str]:
    ...
    if len(argv)!=argc or any(not isinstance(x,str) for x in argv): raise RuntimeError(f"NATIVE_ARGV_MALFORMED:{pid}")
    return argv
